...
async function getBooksAPI(search) {
  const searchedBooks = await axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${search}&key=AIzaSyDmya2Az9xeHL5r5HgyOL29s2hRTyUixLM`);
  return searchedBooks.data.items;
}
...
function getGoogleBook(book) {
  ...
  var newBook = {
    ...
    "publishedDate": date.getFullYear(),
    ...
    "listPrice": {
      "amount": (book.saleInfo.listPrice) ? book.saleInfo.listPrice.amount : 0,
...
function getEmptyBook() {
  ...
  var newBook = {
    ...
    "authors": ["Unknown"],
    "publishedDate": 2020,
    "description": 'placerat nisi sodales suscipit tellus tincidunt mauris elit sit luctus interdum ad',
    "pageCount": 0,
    "categories": [],
    "thumbnail": "https://images.unsplash.com/photo-1510172951991-856a654063f9?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=634&q=80",
    "language": 'en',
    "listPrice": {
      "amount": 0,
      "currencyCode": "ILS",
...
async function getNextPrev(bookId) {
...
async function addReview(bookId, reviewObj) {
  const book = await getById(bookId);
  reviewObj.id = Utils.getRandomId();
  if (!book.review) book.review = [reviewObj]
  else book.review.push(reviewObj)
...
function _createBook(brand, alc) {
  return {
    brand,
    alc,
  }
